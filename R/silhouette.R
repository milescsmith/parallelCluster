...
silhouette.default <- function(x, dist, dmatrix, ...) {
  ...
  if (doRecode) {
    x <- as.integer(fx <- factor(x))
  } # now *has* values in 1:k
...
print.summary.silhouette <- function(x, ...) {
  ...
  cat(
    "Silhouette of", sum(csiz), "units in", k, "clusters",
...
plot.silhouette <-
  function(x, nmax.lab = 40, max.strlen = 5,
    ...
    y <- barplot(s,
      space = space, names = names, xlab = xlab,
