...
clusGap <- function (x, FUNcluster, K.max, B = 100, d.power = 1, spaceH0 = c("scaledPCA", "original"), verbose = interactive(), parallel = FALSE, BPPARAM = bpparam(), ...)
{
    ...
    if(verbose) cat("Bootstrapping, b = 1,2,..., B (= ", B,
                    ")  [one \".\" per sample]:\n", sep="")
    logWksList <- apply_fun(X = 1:B, BPPARAM=BPPARAM, FUN = function(b) {
